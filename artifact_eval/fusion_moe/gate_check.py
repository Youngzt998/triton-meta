...
Run on the 2026 shapes and on the three epilogues this directory adds -- `rweight`, `swiglu_cw`
and `relu2` -- because a gate that was only ever shown to be sensitive on the old epilogues says
nothing about the new ones.

    CUDA_VISIBLE_DEVICES=3 PYTHONPATH=<repo> .venv/bin/python artifact_eval/fusion_moe/gate_check.py
"""
...
from fused_moe import (EPI_CODES, EPI_NEEDS_G, EPI_NEEDS_R, EPI_NEEDS_W, apply_epi,  # noqa: E402
                       fused_plain, out_dtype_for, run_epi)
from run_moe import epi_scale, side_tensor, weight_vector  # noqa: E402
from models_2026 import Case  # noqa: E402


@triton.jit
def _split_acc_gemm(A, B, C, G, R, W, M, N, K, am, ak, bk, bn, cm, cn, gm, gn, s, es, lim, EPI: tl.constexpr,
...
def split_acc(a, b, epi, g=None, r=None, w=None, escale=1.0, lim=0.0, BM=128, BN=128, BK=32):
    M, K = a.shape
    N = b.shape[1]
    ...
    _split_acc_gemm[(triton.cdiv(M, BM) * triton.cdiv(N, BN), )](a, b, out, g if g is not None else out,
                                                                 ...
                                                                 escale, lim, EPI=epi, NEED_G=epi in EPI_NEEDS_G,
                                                                 NEED_R=epi in EPI_NEEDS_R, NEED_W=epi in EPI_NEEDS_W,
                                                                 BM=BM, BN=BN, BK=BK, ODT=tl.float16, num_warps=8)
    return out


# One case per epilogue, on the 2026 shapes the sweep actually reports.  Token counts are the
# even routing load at 16384 total tokens.
CASES = [
    Case("Qwen3.8-2.4T-A95B", "expert.down_proj T16k/even", "moe_down", 320, 8192, 2048, "rweight"),
    Case("gpt-oss-120b", "expert.down_proj T16k/even", "moe_down", 512, 2880, 2880, "rweight"),
    Case("MiniMax-M2", "expert.up_proj T16k/even", "moe_up", 512, 1536, 3072, "swiglu"),
    Case("DeepSeek-V4-Flash", "expert.up_proj T16k/even", "moe_up", 384, 2048, 4096, "swiglu_cw", lim=10.0),
    Case("Nemotron-3.5-L-30B-A3B", "expert.up_proj T16k/even", "moe_up", 768, 1856, 2688, "relu2"),
    Case("Qwen3.8-2.4T-A95B", "o_proj.lora_B r=16", "lora", 4096, 8192, 16, "lora"),
    Case("Qwen3.8-27B", "o_proj", "attn", 4096, 5120, 6144, "resid"),
    Case("gpt-oss-120b", "qkv_proj", "attn", 4096, 5120, 2880, "fp8q"),
]
REPS = 10
...
def main():
    ...
    print(f"{'case':70s} {'correct':>9s} {'no-round':>9s} {'split-acc':>10s}")
    print("-" * 102)
    bad = 0
    for case in CASES:
        ...
        for i in range(REPS):
            ...
            w = weight_vector(M, i, seed) if epi in EPI_NEEDS_W else None
            lim = case.lim
            ref = digest(torch, run_epi(cublas_matmul(a, b, torch.float16), g, r, es, epi, w=w, lim=lim))
            got = [
                fused_plain(a, b, torch.float16, epi, g=g, r=r, escale=es, cfg=(128, 256, 8, 4), SUBTILE=False, w=w,
                            lim=lim),
                fused_plain(a, b, torch.float16, epi, g=g, r=r, escale=es, cfg=(128, 256, 8, 4), SUBTILE=False,
                            ROUND=False, w=w, lim=lim),
                split_acc(a, b, epi, g=g, r=r, w=w, escale=es, lim=lim) if splittable else None,
            ]
        ...
        print(f"{case.label():70s} {counts[0]:5d}/{REPS:<3d} {counts[1]:5d}/{REPS:<3d} {sa}")
        if counts[0] != REPS:
            bad += 1
