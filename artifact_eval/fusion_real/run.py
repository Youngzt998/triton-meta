...
Every case names a model and a layer (see `models.py`) and carries the epilogue that actually
follows that GEMM in that model.

Usage:
    CUDA_VISIBLE_DEVICES=3 PYTHONPATH=/home/youngzt/bitwise-equiv/triton \
        .venv/bin/python artifact_eval/fusion_real/run.py --group lora --reps 10
"""
...
from fused_real import (EPI_ARGMAX, EPI_CODES, EPI_FP8_OUT, EPI_NEEDS_G, EPI_NEEDS_R,  # noqa: E402
                        free_argmax, free_fused, fused_argmax, fused_plain, fused_plain_notma, out_dtype_for,
                        run_argmax, run_epi)
from models import Case, build  # noqa: E402
...
def one_pointwise(case, epi, args, flush, rec):
    ...
    def run_fused(c, ai, bi, gi, ri, o=None):
        if c[0] == "tma":
            return fused_plain(ai, bi, dt, epi, g=gi, r=ri, escale=es, out=o, cfg=c[1][:4], SUBTILE=c[1][4])
        return fused_plain_notma(ai, bi, dt, epi, g=gi, r=ri, escale=es, out=o, cfg=c[1])
    ...
    for i in range(args.reps):
        ...
        base_i = run_epi(c_i, gi, ri, es, epi, cfg=epi_cfg)
        fus_i = run_fused(fcfg, ai, bi, gi, ri)
        torch.cuda.synchronize()
        ...
        if i == 0:
            ...
            del hh
        del ai, bi, gi, ri, base_i, fus_i, c_i
    ...
    t_ours_unfused = graph_ms(torch,
                              lambda: run_epi(cublas_equivalent_gemm(a, b, dt), g, r, es, epi, out=obuf, cfg=epi_cfg),
                              flush)
    ...
    t_free, free_cfg = _tune(FREE_SPACE, lambda c: (lambda: free_fused(a, b, c, epi, g=g, r=r, escale=es, out=obuf)),
    ...
    def build_noround(c):
        if c[0] == "tma":
            return lambda: fused_plain(a, b, dt, epi, g=g, r=r, escale=es, out=obuf, cfg=c[1][:4], SUBTILE=c[1][4],
                                       ROUND=False)
        return lambda: fused_plain_notma(a, b, dt, epi, g=g, r=r, escale=es, out=obuf, cfg=c[1], ROUND=False)
...
def one(case: Case, args, flush):
    ...
    rec = {
        ...
        case.N, "K": case.K, "epi": case.epi, "note": case.note, "dtype": "fp16", "when": time.strftime("%H:%M:%S")
    }
...
def show(rec):
    head = f"  {rec['model']:12s} {rec['layer']:20s} {rec['M']:>5}x{rec['N']:<6}x{rec['K']:<6} {rec['epi']:7s}"
    if "skip" in rec:
...
def main():
    ...
    ap.add_argument("--group", default="lora", help="comma-separated: lora,fp8q,lmhead,trunk")
