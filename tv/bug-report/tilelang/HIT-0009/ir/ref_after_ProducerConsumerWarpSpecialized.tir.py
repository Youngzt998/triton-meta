...
class Module:
    ...
    def main(Q_handle: T.handle, K_handle: T.handle, V_handle: T.handle, O_handle: T.handle):
        ...
        with T.sblock("tilelang_root"):
            ...
            T.sblock_attr({"barrier_init": {mbarrier: [1, 1, 1, 1, 128, 128, 128, 128, 1]}})
            Qs = T.sblock_alloc_buffer((64, 64), "float16", scope="shared.dyn")
            Ks = T.sblock_alloc_buffer((2, 64, 64), "float16", scope="shared.dyn")
            Vs = T.sblock_alloc_buffer((2, 64, 64), "float16", scope="shared.dyn")
            ...
            mbarrier_1 = T.sblock_alloc_buffer((9,), "uint64", data=mbarrier, scope="shared.barrier")
            T.tma_copy(T.region(Q[0, by, bx * 64, 0], 1, 1, 1, 64, 64), T.region(Qs[0, 0], 2, 64, 64), barrier=mbarrier_1[8], is_tma_copy=1, emit_arrive=1)
            T.attr([128, 128], "kWarpSpecializationScope", 0)
            if tx < 128:
                for k in range(4):
                    T.mbarrier_wait_parity(mbarrier_1[4 + k % 2], T.bitwise_xor(k // 2 % 2, 1))
                    T.tma_copy(T.region(K[0, by, k * 64, 0], 1, 1, 1, 64, 64), T.region(Ks[k % 2, 0, 0], 2, 1, 64, 64), barrier=mbarrier_1[k % 2], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.mbarrier_wait_parity(mbarrier_1[6 + k % 2], T.bitwise_xor(k // 2 % 2, 1))
                    T.tma_copy(T.region(V[0, by, k * 64, 0], 1, 1, 1, 64, 64), T.region(Vs[k % 2, 0, 0], 2, 1, 64, 64), barrier=mbarrier_1[2 + k % 2], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=k // 2 % 2)
            else:
                T.fill(T.region(o[0, 0], 2, 64, 64), 0)
                T.fill(T.region(ls[0], 2, 64), 0)
                T.fill(T.region(mx[0], 2, 64), T.infinity("float32") * T.float32(-1.0))
                for k in range(4):
                    T.fill(T.region(s[0, 0], 2, 64, 64), 0)
                    if k == 0:
                        T.mbarrier_wait_parity(mbarrier_1[8], 0)
                    T.mbarrier_wait_parity(mbarrier_1[k % 2], k // 2 % 2)
                    T.gemm(T.region(Qs[0, 0], 1, 64, 64), T.region(Ks[k % 2, 0, 0], 1, 1, 64, 64), T.region(s[0, 0], 3, 64, 64), T.bool(False), T.bool(True), 64, 64, 64, 1, T.bool(False), 64, 64, 0, 0, 1, 0, 0, 0, 0, tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.ptx_arrive_barrier(mbarrier_1[4 + k % 2])
                    T.copy(T.region(mx[0], 1, 64), T.region(mp[0], 2, 64), tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.fill(T.region(mx[0], 2, 64), T.infinity("float32") * T.float32(-1.0))
                    ...
                    T.copy(T.region(s[0, 0], 1, 64, 64), T.region(sc[0, 0], 2, 64, 64), tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.mbarrier_wait_parity(mbarrier_1[2 + k % 2], k // 2 % 2)
                    T.gemm(T.region(sc[0, 0], 1, 64, 64), T.region(Vs[k % 2, 0, 0], 1, 1, 64, 64), T.region(o[0, 0], 3, 64, 64), T.bool(False), T.bool(False), 64, 64, 64, 1, T.bool(False), 64, 64, 0, 0, 1, 0, 0, 0, 0, tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.ptx_arrive_barrier(mbarrier_1[6 + k % 2])
