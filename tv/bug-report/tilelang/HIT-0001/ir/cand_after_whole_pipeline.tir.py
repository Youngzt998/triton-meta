...
class Module:
    ...
    def main_kernel(decoded_values: T.handle("float32", "global"), packed_values: T.handle("uint32", "global")):
        T.func_attr({"calling_conv": 2, "target": T.target({"arch": "sm_90a", "keys": ["cuda", "gpu"], "kind": "cuda", "max_num_threads": 1024, "tag": "", "thread_warp_size": 32}), "thread_extent": {"blockIdx.x": T.int64(1), "threadIdx.x": T.int64(1), "threadIdx.y": T.int64(1), "threadIdx.z": T.int64(1)}, "tirx.is_global_func": T.bool(True), "tirx.kernel_launch_params": ["blockIdx.x", "threadIdx.x", "threadIdx.y", "threadIdx.z"], "tirx.noalias": True, "tl.non_restrict_params": [], "tl.readonly_param_indices": [1]})
        packed_values_1 = T.decl_buffer((2,), "uint32", data=packed_values)
        decoded_values_1 = T.decl_buffer((7,), data=decoded_values)
        bx = T.launch_thread("blockIdx.x", T.int64(1))
        tx = T.launch_thread("threadIdx.x", T.int64(1))
        ty = T.launch_thread("threadIdx.y", T.int64(1))
        tz = T.launch_thread("threadIdx.z", T.int64(1))
        for i in range(T.int64(7)):
            decoded_values_1[i] = T.Cast("float32", T.shift_right(T.shift_left(T.Cast("int64", T.bitwise_and(T.shift_right(packed_values_1[i // T.int64(4)], T.Cast("uint32", i % T.int64(4) * T.int64(8))), T.uint32(255))), T.int64(24)), T.int64(24)))
    ...
    def main(self_handle: T.handle, args: T.handle, num_args: T.int32, result: T.handle("void", "global")) -> T.int32:
        ...
        with T.attr(0, "compute_scope", "main_compute_"):
            T.call_packed("main_kernel", decoded_values, packed_values, T.int64(1), T.int64(1), T.int64(1), T.int64(1))
        return 0