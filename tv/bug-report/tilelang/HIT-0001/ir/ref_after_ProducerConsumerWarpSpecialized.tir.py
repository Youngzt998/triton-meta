...
class Module:
    ...
    def main(Q_handle: T.handle, K_handle: T.handle, V_handle: T.handle, O_handle: T.handle):
        ...
        with T.sblock("tilelang_root"):
            ...
            T.tma_copy(T.region(Q[0, 0, bx * 64, 0], 1, 1, 1, 64, 64), T.region(Qs[0, 0], 2, 64, 64), barrier=mbarrier_1[8], is_tma_copy=1, emit_arrive=1)
            T.attr([128, 128], "kWarpSpecializationScope", 0)
            if tx < 128:
                for k in range(4):
                    T.mbarrier_wait_parity(mbarrier_1[4 + k % 2], T.bitwise_xor(k // 2 % 2, 1))
                    T.tma_copy(T.region(K[0, 0, k * 64, 0], 1, 1, 1, 64, 64), T.region(Ks[k % 2, 0, 0], 2, 1, 64, 64), barrier=mbarrier_1[k % 2], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=k // 2 % 2)
                    T.mbarrier_wait_parity(mbarrier_1[6 + k % 2], T.bitwise_xor(k // 2 % 2, 1))
                    T.tma_copy(T.region(V[0, 0, k * 64, 0], 1, 1, 1, 64, 64), T.region(Vs[k % 2, 0, 0], 2, 1, 64, 64), barrier=mbarrier_1[2 + k % 2], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=k // 2 % 2)
            else:
