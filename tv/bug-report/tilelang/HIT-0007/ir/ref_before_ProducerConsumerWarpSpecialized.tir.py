...
class Module:
    ...
    def main(Q_handle: T.handle, K_handle: T.handle, V_handle: T.handle, O_handle: T.handle):
        ...
        with T.sblock("tilelang_root"):
            ...
            T.copy(T.region(Q[0, by, bx * 64, 0], 1, 1, 1, 64, 64), T.region(Qs[0, 0], 2, 64, 64))
            T.fill(T.region(o[0, 0], 2, 64, 64), 0)
            T.fill(T.region(ls[0], 2, 64), 0)
            T.fill(T.region(mx[0], 2, 64), T.infinity("float32") * T.float32(-1.0))
            for k in T.serial(4, annotations={"num_stages": 1}):
                T.copy(T.region(K[0, by, k * 64, 0], 1, 1, 1, 64, 64), T.region(Ks[0, 0], 2, 64, 64))
                ...
                T.copy(T.region(V[0, by, k * 64, 0], 1, 1, 1, 64, 64), T.region(Vs[0, 0], 2, 64, 64))
