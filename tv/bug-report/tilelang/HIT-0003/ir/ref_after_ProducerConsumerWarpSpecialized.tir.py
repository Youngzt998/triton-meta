...
class Module:
    ...
    def main(Q_handle: T.handle, K_handle: T.handle, V_handle: T.handle, O_handle: T.handle):
        ...
        with T.sblock("tilelang_root"):
            ...
            T.tma_copy(T.region(Q[0, 0, bx * 64, 0], 1, 1, 1, 64, 64), T.region(Qs[0, 0], 2, 64, 64), barrier=mbarrier_1[4], is_tma_copy=1, emit_arrive=1)
            ...
            if tx < 128:
                for k in range(bx + 3):
                    ...
                    T.tma_copy(T.region(K[0, 0, k * 64, 0], 1, 1, 1, 64, 64), T.region(Ks[0, 0], 2, 64, 64), barrier=mbarrier_1[T.FloorMod(k, 1)], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=T.FloorDiv(k, 1) % 2)
                    T.mbarrier_wait_parity(mbarrier_1[3 + T.FloorMod(k, 1)], T.bitwise_xor(T.FloorDiv(k, 1) % 2, 1))
                    T.tma_copy(T.region(V[0, 0, k * 64, 0], 1, 1, 1, 64, 64), T.region(Vs[0, 0], 2, 64, 64), barrier=mbarrier_1[1 + T.FloorMod(k, 1)], is_tma_copy=1, emit_arrive=1, tl.pipeline_mbar_phase_expr=T.FloorDiv(k, 1) % 2)
            else:
