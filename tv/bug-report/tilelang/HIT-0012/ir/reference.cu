...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap O_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (tl::tl_shuffle_elect<256>()) {
    ...
    tl::tma_load(Q_desc, mbarrier[4], (&(((half_t*)Qs)[0])), 0, (((int)blockIdx.x) * 64), ((int)blockIdx.y), 0);
  }
  __syncthreads();
  if (((int)threadIdx.x) < 128) {
    tl::warpgroup_reg_dealloc<24>();
    for (int k = 0; k < min(4, (((int)blockIdx.x) + 3)); ++k) {
      mbarrier[2].wait(((k & 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[0].arrive_and_expect_tx(8192);
        tl::tma_load(K_desc, mbarrier[0], (&(((half_t*)Ks)[0])), 0, (k * 64), ((int)blockIdx.y), 0);
      }
      mbarrier[3].wait(((k & 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[1].arrive_and_expect_tx(8192);
        tl::tma_load(V_desc, mbarrier[1], (&(((half_t*)Vs)[0])), 0, (k * 64), ((int)blockIdx.y), 0);
      }
    }
  } else {
    ...
    *(float2*)(mx + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k_1 = 0; k_1 < min(4, (((int)blockIdx.x) + 3)); ++k_1) {
      #pragma unroll
      for (int i_1 = 0; i_1 < 32; ++i_1) {
        float condval;
        if ((((((k_1 * 64) + ((i_1 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_1 & 1)) <= (((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_1 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) + 62))) {
