...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap O_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (tl::tl_shuffle_elect<256>()) {
    ...
    tl::tma_load(Q_desc, mbarrier[8], (&(((half_t*)Qs)[0])), 0, (((int)blockIdx.x) * 64), ((int)blockIdx.y), 0);
  }
  __syncthreads();
  if (((int)threadIdx.x) < 128) {
    tl::warpgroup_reg_dealloc<24>();
    for (int k = 0; k < 4; ++k) {
      mbarrier[((k & 1) + 4)].wait(((k >> 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[(k & 1)].arrive_and_expect_tx(8192);
        tl::tma_load(K_desc, mbarrier[(k & 1)], (&(((half_t*)Ks)[((k & 1) * 4096)])), 0, (k * 64), ((int)blockIdx.y), 0);
      }
      mbarrier[((k & 1) + 6)].wait(((k >> 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[((k & 1) + 2)].arrive_and_expect_tx(8192);
        tl::tma_load(V_desc, mbarrier[((k & 1) + 2)], (&(((half_t*)Vs)[((k & 1) * 4096)])), 0, (k * 64), ((int)blockIdx.y), 0);
      }
    }
  } else {
    ...
    *(float2*)(mx + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k_1 = 0; k_1 < 4; ++k_1) {
      ...
      for (int i_1 = 0; i_1 < 8; ++i_1) {
        ...
        *(float4*)(s + (i_1 * 4)) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
      }
      if (k_1 == 0) {
        mbarrier[8].wait(0);
      }
      mbarrier[(k_1 & 1)].wait((k_1 >> 1));
      {
        ...
        tl::increase_descriptor_offset<int>(desc_b, ((k_1 & 1) * 8192));
    ...
    for (int i_10 = 0; i_10 < 4; ++i_10) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)Os)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_10 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_10 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)o[(i_10 * 8)]), ((half_t)o[((i_10 * 8) + 1)])), __pack_half2(((half_t)o[((i_10 * 8) + 2)]), ((half_t)o[((i_10 * 8) + 3)])), __pack_half2(((half_t)o[((i_10 * 8) + 4)]), ((half_t)o[((i_10 * 8) + 5)])), __pack_half2(((half_t)o[((i_10 * 8) + 6)]), ((half_t)o[((i_10 * 8) + 7)])));
    }
    tl::__sync_thread_partial(3, 128);
    if (tl::tl_shuffle_elect<128>()) {
