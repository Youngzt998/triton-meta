...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, half_t* __restrict__ O, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (tl::tl_shuffle_elect<256>()) {
    ...
    tl::tma_load(Q_desc, mbarrier[8], (&(((half_t*)Qs)[0])), 0, (((int)blockIdx.x) * 64), 0, 0);
  }
  __syncthreads();
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(K_desc, mbarrier[(k & 1)], (&(((half_t*)Ks)[((k & 1) * 4096)])), 0, (k * 64), 0, 0);
      }
      mbarrier[((k & 1) + 6)].wait(((k >> 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[((k & 1) + 2)].arrive_and_expect_tx(8192);
        tl::tma_load(V_desc, mbarrier[((k & 1) + 2)], (&(((half_t*)Vs)[((k & 1) * 4096)])), 0, (k * 64), 0, 0);
      }
    }
  } else {
