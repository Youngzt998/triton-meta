# [SUPERSEDED / HISTORICAL] — old tv/ CLAUDE.md

> ⚠️ **This file is the OLD guidance and is SUPERSEDED by `tv/CLAUDE.md`.**
> Kept only for history. Some design claims below are **stale** and were changed
> later (e.g. triton-tv "logic TODO", "semantics/mlir empty", a single
> `globalMem`, the store contract, GoogleTest tests). **Do not follow this file
> and do not treat it as current memory.** For current guidance see
> `tv/CLAUDE.md`; for the refactor direction see
> `tv/doc/tensor-languages-survey.md`.

---

# CLAUDE.md — `tv/` Translation Validator  *(original content, historical)*

This file provides guidance to Claude Code (claude.ai/code) when working on the
translation validator under `tv/`. All development for this work stays inside
`tv/` and is pushed only to the `tv` or `tv-trials` branches. The repo-wide
Triton guidance lives in the root `CLAUDE.md`; the sections below cover the
build, test, and debug commands that matter for `tv/` work plus the validator's
own design.
